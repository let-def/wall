...
let draw_eyes vg xf x y w h mx my t =
  ...
  C.new_path vg;
  C.ellipse vg xf ~cx:(lx +. 3.0) ~cy:(ly +. 16.0) ~rx:ex ~ry:ey;
  C.ellipse vg xf ~cx:(rx +. 3.0) ~cy:(ry +. 16.0) ~rx:ex ~ry:ey;
  ...
  C.new_path vg;
  C.ellipse vg xf ~cx:lx ~cy:ly ~rx:ex ~ry:ey;
  C.ellipse vg xf ~cx:rx ~cy:ry ~rx:ex ~ry:ey;
  C.fill vg (Paint.linear_gradient
  ...
  let dy = dy *. ey *. 0.5 in
  C.new_path vg;
  C.ellipse vg xf ~cx:(lx +. dx) ~cy:(ly +. dy +. ey *. 0.25 *. (1.0 -. blink)) ~rx:br ~ry:(br *. blink);
...
let draw_graph vg xf x y w h t =
  ...
  C.line_to vg xf ~x:(x +. w) ~y:(y +. h);
  C.line_to vg xf ~x ~y:(y +. h);
  ...
  for i = 0 to 5 do
    C.new_path vg;
    C.rect vg xf ~x:(sx i -. 10.0) ~y:(sy i -. 10.0 +. 2.0) ~w:20.0 ~h:20.0;
    C.fill vg
  ...
  C.new_path vg;
  for i = 0 to 5 do
    C.circle vg xf ~cx:(sx i) ~cy:(sy i) ~r:4.0;
  done;
  C.fill vg (Paint.color (Color.v 0.0 0.6 0.75 1.0));
  C.new_path vg;
  for i = 0 to 5 do
    C.circle vg xf ~cx:(sx i) ~cy:(sy i) ~r:2.0;
  done;
...
let draw_spinner vg xf cx cy r t =
  ...
  let r1 = r *. 0.75 in
  C.new_path vg;
  C.arc vg xf ~cx ~cy ~r:r0 ~a0:a0 ~a1:a1 `CW;
  C.arc vg xf ~cx ~cy ~r:r1 ~a0:a1 ~a1:a0 `CCW;
  C.close_path vg;
