...
let draw_colorwheel vg xf x y w h t =
  ...
  C.stroke vg xf path
    (Paint.color (Color.gray ~a:0.25 0.0)) Outline.default;
...
let draw_widths vg xf x y w =
  ...
  for i = 0 to 19 do
    ...
    C.stroke vg xf
      ...
      paint Outline.{default with stroke_width = (float i +. 0.5) /. 10.0};
    y := !y +. 10.0;
  done
...
let draw_caps vg xf x y w =
  ...
  for i = 0 to 2 do
    C.stroke vg xf
      ...
      Paint.black Outline.{default with stroke_width; line_cap = caps.(i)};
  done
...
let draw_scissor vg xf x y t =
  ...
  C.fill vg xf
    (C.path @@ fun t -> P.rect t (-20.0) (-20.0) (60.0) (40.0))
  ...
  let path = C.path @@ fun t -> P.rect t (-20.0) (-10.0) 60.0 30.0 in
  C.fill vg xf path (Paint.color (Color.v 1.0 0.5 0.0 0.25));
  (* Draw second rectangle with combined scissoring. *)
  let frame = Frame.intersect_scissor (-20.0) (-10.0) 60.0 30.0 xf frame in
  C.fill vg xf path ~frame (Paint.color (Color.v 1.0 0.5 0.0 1.0))

let draw_window vg xf title x y w h =
  let cornerRadius = 3.0 in
  (* Window *)
  C.fill vg xf
    (C.path @@ fun t -> P.round_rect t x y w h cornerRadius)
  ...
  C.stroke vg xf
    ...
    (Paint.color (Color.gray ~a:0.125 0.0)) Outline.default;
...
let draw_searchbox vg xf text x y w h =
  ...
  C.stroke vg xf
    ...
    (Paint.color (Color.gray ~a:0.2 0.0)) Outline.default;
...
let draw_dropdown vg xf text x y w h =
  ...
  C.stroke vg xf
    (C.path @@ fun t ->
     P.round_rect t (x+.0.5) (y+.0.5) (w-.1.0) (h-.1.0) (cornerRadius-.0.5))
    (Paint.color (Color.gray ~a:0.1875 0.0)) Outline.default;
...
let draw_button vg xf preicon text x y w h col =
  ...
  in
  if is_black then (
    C.fill vg xf path (Paint.color col);
  );
  C.fill vg xf path
    (Paint.linear_gradient x y x (y+.h)
  ...
  C.stroke vg xf
    (C.path @@ fun t ->
     P.round_rect t (x+.0.5) (y+.0.5) (w-.1.0) (h-.1.0) (cornerRadius-.0.5))
    (Paint.color (Color.gray ~a:0.375 0.0)) Outline.default;
...
let draw_slider vg xf pos x y w h =
  ...
  in
  C.fill vg xf path (Paint.color (Color.v_srgbi 40 43 48));
  C.fill vg xf path (Paint.linear_gradient x (cy-.kr) x (cy+.kr)
